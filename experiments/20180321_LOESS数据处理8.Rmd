...
```{r}
loess1 <- loess(tml1001$机动车当量~tml1001$时间序号,span=0.8)
...
handleloess <- function(df,exp,resp){
  ...
  result <- loess(df[,resp]~df[,exp],span=0.8)
...
组间平方和/总平方和指标从39.2%（无LOESS）提升到了66.48%（SPAN=0.1），再提升到71.19%（SPAN=0.2），再提升到73.71%（SPAN=0.3），再提升到75.70%（SPAN=0.4），再提升到76.92%（SPAN=0.5），再提升到78.48%（SPAN=0.6）,再提升到79.90%（SPAN=0.7），再提升到81.01%（SPAN=0.8）。可见LOESS有效果。
...
ggplot(tmlallloess,aes(tmlallloess$时间序号,tmlallloess$loess,group=tmlallloess$日期,color=tmlallloess$group_k2))+
  ...
  xlab("时间序号")+ylab("车流量")+scale_color_hue("类别簇")+labs(title="SPAN=0.8")+
  scale_x_continuous(breaks = seq(0,288,24),limits = c(0,288))+
  scale_y_continuous(breaks = seq(0,120,20))

ggsave("D:\\王致远\\论文\\大论文\\实验\\绘图\\span0.8.jpg",width=7.29,height=4.5,dpi=600)
```
