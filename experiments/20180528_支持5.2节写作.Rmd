...
pre1006 <- TFTSA::flow_knn(tmlobj,tmlbase,start = 73,k = 3,lag_duration = 24,fore_duration = 12,
                           save_detail = "D:\\王致远\\论文\\大论文\\实验\\detail1.csv")
```
...
```{r}
ggsave("D:\\王致远\\论文\\大论文\\实验\\绘图\\K近邻原始数据预测.jpg",width=7.29,height=4.5,dpi=600)
```
...
```{r}
pre1006 <- TFTSA::flow_knn(tmlobj,tmlbase,start = 73,k = 3,lag_duration = 24,fore_duration = 12,
                           save_detail = "D:\\王致远\\论文\\大论文\\实验\\detail2.csv")
```
...
```{r}
ggsave("D:\\王致远\\论文\\大论文\\实验\\绘图\\K近邻LOESS数据预测.jpg",width=7.29,height=4.5,dpi=600)
```
...
```{r}
write.csv(optik,file="D:\\王致远\\论文\\大论文\\实验\\结果表\\optik.csv")
```
...
```{r}
ggsave("D:\\王致远\\论文\\大论文\\实验\\绘图\\K近邻参数ld4.jpg",width=7.29,height=4.5,dpi=600)
```
...
```{r}
write.csv(optild,file="D:\\王致远\\论文\\大论文\\实验\\结果表\\optild.csv")
```
...
pre_fd <- flow_knn(tmlobj,tmlbase,start = 73,k = 3,lag_duration = 24,fore_duration = 27,
                  save_detail = "D:\\王致远\\论文\\大论文\\实验\\结果表\\fd27.csv")
```
